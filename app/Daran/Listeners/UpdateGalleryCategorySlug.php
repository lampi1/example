...
class UpdateGalleryCategorySlug
{
    ...
    public function handle(GalleryCategoryUpdated $event)
    {
        ...
        $old_url = LaravelLocalization::getLocalizedURL($event->category->locale, route('galleries.category',['permalink'=>$old_slug]));
        $new_url = LaravelLocalization::getLocalizedURL($event->category->locale, route('galleries.category',['permalink'=>$current_slug]));

        Redirection::create([
            'name'=>'Gallery Category '.$event->category->id,
            'code'=>'301',
            'from_uri'=>str_replace(config('app.url').'/','',$old_url),
            'to_uri'=>str_replace(config('app.url').'/','',$new_url)
        ]);

        $mr = MenuResource::where('route','galleries.category')->first();
        if(!$mr){
            return;
        }
