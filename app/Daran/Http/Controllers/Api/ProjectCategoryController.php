...
class ProjectCategoryController extends Controller
{
    public function getCategories(Request $request)
    {
        ...
        $qb = ProjectCategory::select('id','name','priority')->when($request->filled('lang'),function($q) use($request){
    ...
    public function reorder(Request $request)
    {
        ...
        $postCategory = ProjectCategory::find($id);
        $postCategory->priority = $new_index;
        $postCategory->save();

        if($old_index < $new_index){
            if(($new_index-$old_index) == 1){
                $postCategories = ProjectCategory::where('priority', '<=',  $new_index)->where('priority', '>=',  $old_index)->where('id', '!=', $request->id)->get();
            }else{
                $postCategories = ProjectCategory::where('priority', '<=',  $new_index)->where('priority', '>',  $old_index)->where('id', '!=', $request->id)->get();
            }
        }else{
    ...
    public function destroy($id)
    {
        ...
        $postCategory = ProjectCategory::findOrFail($id);
