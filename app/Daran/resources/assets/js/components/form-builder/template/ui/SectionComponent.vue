<template>
    <div>
        <div class="row">
            <div class="col-3">
                <select class="v-select" v-model="form.layout">
                    <option value="" disabled selected>Seleziona layout</option>
                    <option v-for="(value, key) in layouts" :value="key">{{value}} layout</option>
                </select>
            </div>
            <div class="col-9 text-right">
                <button class="btn btn-primary" @click="addSection">Aggiungi Sezione</button>
        ...
        <div id="sectionWrapper" class="mt-4">
            <div class="w-100 mt-2 accordion sectionItem" v-for="(section, index) in form.sections" :id="section.name" :key="section.name">
                <div class="card">
                    <div class="card-header">
                        <div class="row">
                            <div class="col-5">
                                <input class="w-100" type="text" placeholder="Etichetta Sezione" v-model="section.label">
                            </div>
                            <div class="col-7 text-right">
                                <p style="margin-top: 5px;">
                                    <span class="btn btn-sm btn-primary pr-2 clickable" @click="addRow(index)"><font-awesome-icon icon="plus"/> Aggiungi Riga</span>
                                    <span class="btn btn-sm btn-primary pr-2 clickable" @click="delSection(index)"><font-awesome-icon icon="times"/> Rimuovi Sezione</span>
                                    <span class="btn btn-sm btn-primary pr-2 clickable" @click="configSection(index)"><font-awesome-icon icon="cog"/> Configura Sezione</span>
                                    <span class="clickable collapsed" data-toggle="collapse" :data-target="'#' + section.name + '_body'">
                                        <i class="fc--white fa fa-fw fa-chevron-up"></i>
                                        <i class="fc--white fa fa-fw fa-chevron-down"></i>
                                    </span>
                                </p>
                            </div>
                        </div>
