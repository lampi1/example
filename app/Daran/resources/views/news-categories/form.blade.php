                        ...
                        <tbody>
                            @foreach($category->translations as $category_trans)
                                <tr>
                                    ...
                                    @can('edit news')
                                        <td class="text-right">
                                            <a class="ico" href="{{ route('admin.news-categories.edit', ['id' => $category_trans->id]) }}" data-icon="N" title="@lang('daran::common.edit')" data-tooltip="tooltip"></a>
                                            @can('delete news')
                            ...
                            @foreach(config('app.available_translations') as $trans)
                                @if ($category->locale != $trans && !$category->translations->contains('locale',$trans))
                                    <tr>
                                        ...
                                        @can('create news')
                                            <td class="text-right">
                                                <a class="ico" href="{{ route('admin.news-categories.create', ['locale' => $trans, 'locale_group' => $category->locale_group]) }}" data-icon="F" title="@lang('daran::common.create')" data-tooltip="tooltip"></a>
                                            </td>
                                        @endcan
                                    </tr>
                                @endif
